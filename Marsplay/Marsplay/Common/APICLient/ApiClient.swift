...
//  Marsplay
//
//  Created by Amol Prakash on 23/01/20.
...
enum ApiKey {
    ...
    static let apiKeyValue = "eeefc96f"
}
...
enum Endpoints {
    enum Movies: Endpoint {
        ...
        public var path: String {
            switch self {
            case .fetch: return "\(EnvironmentURL.baseUrl)"
            }
        }
...
class ApiClient<T: Decodable> {
    ...
    static func makeRequest(toURL url: String,
                     ...
                     completion: @escaping (_ value: T?, _ error: MarsplayError?) -> Void) {
        guard Reachability.isConnectedToNetwork() else {
            completion(nil, MarsplayError.noInternet)
            return
        }
    ...
    static private func add(queryParams: [String: String]?, toURL url: URL) -> URL {
        guard var queryParams = queryParams, queryParams.count > 0 else {
            return url
        }
        queryParams[ApiKey.apiKey] = ApiKey.apiKeyValue
